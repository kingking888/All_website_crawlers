...
class LagouSpider(object):
    ...
    def save_data(self, items):
        # 获取文件大小
        file_size = os.path.getsize(r'position2.csv')
        if file_size == 0:
            # 表头
            ...
            # 数据写入
            file_test.to_csv(r'position2.csv', encoding='utf-8', index=False)
        else:
            with open(r'position2.csv', 'a+', newline='',encoding="utf-8") as file_test:
                # 追加到文件后面
    ...
    def post_target_url(self):
        ...
        pn = 1
        proxies = {"http":"110.52.235.54:9999",
                   "https":"110.52.235.54:9999"}
        for pg in range(22):
            formdata = {
                'first': 'false',
                'pn': pn,
